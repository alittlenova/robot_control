...
uint32_t A_Xout,A_Yout,A_Zout;
uint32_t A_X,A_Y,A_Z;
//short data_xyz[3];
#define TIME_OUT   0x000FFFFF
#define   uchar unsigned char
uint32_t count_wait = TIME_OUT;
//uint8_t ge,shi,bai,qian,wan,shiwan;    
static uint8_t Error_CallBack(uint8_t code);
static uint32_t Send_ADXL345_data(uint32_t dis_data);
static void ADXL_generate_data(void);
void ADXL_generate_data(void)
{
	A_Xout=Send_ADXL345_data(A_X);
	A_Yout=Send_ADXL345_data(A_Y);
	A_Zout=Send_ADXL345_data(A_Z);
}
...
uint32_t Send_ADXL345_data(uint32_t dis_data)
  { uint32_t temp ;
		uint32_t positive;
