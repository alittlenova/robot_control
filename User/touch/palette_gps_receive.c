...
#include "./touch/palette_gps_debug.h"
...
* @retval 无
*/
void GPS_RECEIVE_Palette_Init(void)
{
  ...
  // 初始化按钮
	button_n=9;
  GPS_RECEIVE_Touch_Button_Init();
  
  // 描绘按钮
  for(i=8;i<button_n;i++)
  {
    button[i].draw_btn(&button[i]);
  }
  
	GPS_RECEIVE_Display_Info();
	//DISPLAY_Data.Refresh=1;
...
* @retval 无
*/
void GPS_RECEIVE_Touch_Button_Init(void)
{
  //第一列按钮
...
void GPS_RECEIVE_Display_Info(void)
{
			//先清屏
			...
			clear_output_string();
			my_strcat(Output_String,"GPS接收信息 ");
			...
			LCD_SetColors(CL_RED,CL_BUTTON_GREY);
			LCD_SetFont(&Font16x24);
			LCD_DispString_EN_CH( 380,
														200,                     
			Output_String);
			//显示语句2
			clear_output_string();
			my_strcat(Output_String,"GPVTG: ");
			myitoa(GPS_Data.GPVTG_Get, NUMBER, 10);
			my_strcat(Output_String,NUMBER);
			LCD_SetColors(CL_RED,CL_BUTTON_GREY);
			LCD_SetFont(&Font16x24);
			...
			//显示语句3
			clear_output_string();
			my_strcat(Output_String,"GPGGA: ");
			myitoa(GPS_Data.GPGGA_Get, NUMBER, 10);
			my_strcat(Output_String,NUMBER);
			LCD_SetColors(CL_RED,CL_BUTTON_GREY);
			LCD_SetFont(&Font16x24);
			...
			//显示语句4
			clear_output_string();
			my_strcat(Output_String,"GPGLL: ");
			myitoa(GPS_Data.GPGLL_Get, NUMBER, 10);
			my_strcat(Output_String,NUMBER);
			LCD_SetColors(CL_RED,CL_BUTTON_GREY);
			LCD_SetFont(&Font16x24);
			...
			//显示语句6
			clear_output_string();
			my_strcat(Output_String,"GPGSV:");
			myitoa(GPS_Data.GPGSV_Get, NUMBER, 10);
			my_strcat(Output_String,NUMBER);
			LCD_SetColors(CL_RED,CL_BUTTON_GREY);
			LCD_SetFont(&Font16x24);
			...
			//显示语句6
			clear_output_string();
			my_strcat(Output_String,"GPRMC:");
			myitoa(GPS_Data.GPRMC_Get, NUMBER, 10);
			my_strcat(Output_String,NUMBER);
			LCD_SetColors(CL_RED,CL_BUTTON_GREY);
			LCD_SetFont(&Font16x24);
			LCD_DispString_EN_CH( 330,
														200,                     
			Output_String);
}
/* ------------------------------------------end of file---------------------------------------- */
